...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (error instanceof Error && (error.message.includes('Invalid user ID') || error.message.includes('User not found'))) {
          notFound(); // This will render your not-found.tsx component
        } else {
          // For any other unexpected errors, show a generic error message on the page
  ...
  return (
    <div className="space-y-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 backdrop-blur-md h-full bg-white/10 flex items-center justify-center z-50 p-4">
            ...
            <div className="space-y-6">
              {user.posts.length === 0 ? (
                ...
                user.posts.map(post => (
                  <PostCard
                    key={post._id}
                    post={{ ...post, author: { _id: user._id, name: user.name } }}
